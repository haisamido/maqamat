...
generate_all_by_ets: directories ## generate all defined equally tempered chromatic scales
	...
	$(MAKE) --silent generate_by_et TET_INTERVAL=$(TET_INTERVAL) | grep -v 'ing directory ' > $(ET_DIR)/equal_temperament_$(TET_INTERVAL).tsv;)

generate_all_by_ratios: directories
	@$(MAKE) --silent generate_by_ratios  | grep -v 'ing directory ' > $(RATIOS_DIR)/just_intervals.tsv;
	@$(MAKE) --silent generate_by_ratios2 | grep -v 'ing directory ' > $(RATIOS_DIR)/pythagorean_intervals.tsv;
	@$(MAKE) --silent generate_by_ratios3 | grep -v 'ing directory ' > $(RATIOS_DIR)/pythagorean_intervals_turkish.tsv
	@$(MAKE) --silent generate_urmawi     | grep -v 'ing directory ' > $(RATIOS_DIR)/urmawi.tsv
...
just_intervals        ='[1,   16/15, 9/8,   6/5,   5/4, 4/3,   45/32,    64/45, 3/2,    8/5,   5/3,  9/5,    15/8, 2]'
pythagorean_intervals ='[1, 256/243,  9/8, 32/27, 81/64, 4/3, 1024/729, 729/512, 3/2, 128/81, 27/16, 16/9, 243/128, 2]'
...
urmawi                ='[1, 256/243, 65536/59049, 9/8, 32/27, 8192/6561, 81/64, 4/3, 1024/729, 262144/177147, 3/2, 128/81, 32768/19683, 27/16, 16/9, 4096/2187, 1048576/531441, 2]'
...
generate_urmawi: ## generate intervals with urmawi ratios
#	@echo "Generating intervals with urmawi ratios" >&2
	@${PYTHON} ./maqamat.py ${AUDIO} -R -r $(urmawi) -o $(RATIOS_DIR)/urmawi.svg 
